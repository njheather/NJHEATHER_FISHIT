...
-- Safety Check
if not ReplicatedStorage:FindFirstChild("Packages") or not ReplicatedStorage.Packages:FindFirstChild("Replion") then
    return
end
...
-- 4. MAIN UI CREATION
if Players.LocalPlayer.PlayerGui:FindFirstChild("NJHeatherUI") then
    Players.LocalPlayer.PlayerGui.NJHeatherUI:Destroy()
end
...
local function TriggerSell()
    ...
    if successNet then
        local result = net["RF/SellAllItems"]:InvokeServer()
        if result == true then
...
local SellSettings = {
    ...
    DurationMinutes = 0,
    LastDurationSell = tick()
}

-- [[ CHANGED: REAL-TIME COUNT FUNCTION ]] --
local function GetRealTimeFishCount()
    local success, fishData = pcall(function() 
        return Data:GetExpect({"Inventory", "Fish"}) 
    end)
    
    if not success or not fishData then 
        return 0 
    end

    local count = 0
    for _ in pairs(fishData) do
        count = count + 1
    end
...
local function createInputRow(page, labelText, placeholder, callback)
    ...
    local inputFrame = create("Frame", {
        ...
        Position = UDim2.new(1, -240, 0.5, -15),
        BackgroundColor3 = UI_CONFIG.Colors.Black,
    ...
    local function handleUpdate()
        local inputVal = tonumber(inputBox.Text) or 0
        if callback then callback(toggled, inputVal) end
    end
    ...
    inputBox.FocusLost:Connect(function() handleUpdate() end)
...
if Pages["DASHBOARD"] then
    createRow(Pages["DASHBOARD"], "MANUAL SELL", "SELL ALL", function() 
        TriggerSell()
    end)

    createInputRow(Pages["DASHBOARD"], "AUTO SELL (Count)", "Limit", function(active, input)
        SellSettings.CountActive = active
        SellSettings.CountLimit = input
    end)

    createInputRow(Pages["DASHBOARD"], "AUTO SELL (Mins)", "Minutes", function(active, input)
        SellSettings.DurationActive = active
        SellSettings.DurationMinutes = input
        SellSettings.LastDurationSell = tick()
    end)
end

-- [[ CHANGED: REAL-TIME LOOP WITH DEBUG ]] --
task.spawn(function()
    while task.wait(0.5) do -- Fast loop for real-time check
        
        -- Logic: Handle Real-Time Fish Count
        local currentFish = GetRealTimeFishCount()
        
        -- DEBUG PRINT (Check Console F9)
        -- This helps you see if the count works without needing to sell
        if SellSettings.CountActive then
            print("[DEBUG] Fish Count:", currentFish, "/", SellSettings.CountLimit)
        end

        -- Logic: Trigger Sell ONLY if limit reached and Active
        if SellSettings.CountActive and SellSettings.CountLimit > 0 then
            if currentFish >= SellSettings.CountLimit then
                TriggerSell()
                task.wait(2.5) -- Wait for transaction to finish
            end
        end

        -- Logic: Sell By Duration
        if SellSettings.DurationActive and SellSettings.DurationMinutes > 0 then
            local secondsLimit = SellSettings.DurationMinutes * 60
            if (tick() - SellSettings.LastDurationSell) >= secondsLimit then
                TriggerSell()
                SellSettings.LastDurationSell = tick() 
            end
        end
...
local function createItemCard(slider, itemId, name, price, stats, imageId, rawData, category)
    ...
    local constraintStats = Instance.new("UITextSizeConstraint")
    constraintStats.MaxTextSize = 16
    constraintStats.MinTextSize = 10
    constraintStats.Parent = statsLabel
end
...
if StorePage then
    ...
    task.spawn(function()
        ...
        for _, v in ipairs(ItemsFolder:GetChildren()) do
            if v:IsA("ModuleScript") then
                ...
                if s and r and r.Data and (r.Price or 0) > 0 then
                    ...
                    -- [[ KATEGORI 2: UTILITY / GEARS ]] --
                    -- Kita gabungkan cek Radar dan Diving Gear disini
                    elseif v.Name:match("Fishing Radar") or v.Name:match("Diving Gear") then
                        
                        local statsStr = "" -- Kosongkan Stats sesuai request
        ...
        -- 2. LOOP KHUSUS BAITS
        -- Mencari folder bait dengan beberapa kemungkinan nama/lokasi
        ...
        if BaitFolder then
            for _, v in ipairs(BaitFolder:GetChildren()) do
                -- Filter ketat: Pastikan bukan Rod/Radar/Gear yang nyasar
                if v:IsA("ModuleScript") and not v.Name:match("Rod") and not v.Name:match("Radar") and not v.Name:match("Gear") then
                    local s, r = pcall(require, v)
                    
                    -- Cek Type Baits agar aman
                    if s and r and r.Data and (r.Data.Type == "Baits" or r.Type == "Baits") and (r.Price or 0) > 0 then
                        local statsList = {}
                        ...
                        for key, val in pairs(modifiers) do
                            if key ~= "BaseLuck" and not key:lower():match("lure") then
                                local finalVal = val * 100
                                local displayName = key
                                if key:match("Shiny") then displayName = "Shiny Chance"
                                elseif key:match("Fairy") then displayName = "Fairy Dust"
                                elseif key:match("XP") then displayName = "Bonus XP"
                                elseif key:match("Mutation") then displayName = "Mutation Chance"
                                end
        ...
        -- 3. SORTING & RENDERING (JANGAN LUPA BAGIAN INI)
        ...
        for _, item in ipairs(ShopData.Rods) do
            createItemCard(RodSlider, item.Id, item.Name, item.Price, item.Stats, item.Image, item.Raw, "Fishing Rods")
        end
        for _, item in ipairs(ShopData.Baits) do
            createItemCard(BaitSlider, item.Id, item.Name, item.Price, item.Stats, item.Image, item.Raw, "Baits")
        end
        for _, item in ipairs(ShopData.Utility) do
            createItemCard(UtilSlider, item.Id, item.Name, item.Price, item.Stats, item.Image, item.Raw, "Items")
        end
    end)
end
